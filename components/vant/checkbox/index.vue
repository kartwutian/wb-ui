...
<script>
  ...
  export default {
    ...
    computed: {
      iconClasses() {
        return `${utils.bem('checkbox__icon', [this.shape, {disabled: this.isDisabled, checked: this.checked}])}`
      },
      iconStyle() {
        return `font-size: ${this.sizeWithUnit};${this.checkedColor && this.checked && !this.isDisabled ? 'border-color:' + this.checkedColor + '; background-color:' + this.checkedColor : ''}`
      },
      labelClasses() {
        return `${this.labelClass} ${utils.bem('checkbox__label', [this.labelPosition, {disabled: this.isDisabled}])}`
      },
      sizeWithUnit() {
      ...
      isDisabled: {
        get() {
          return this.hasParent ? this.disabled || this.proxyDisabled: this.disabled;
        },
      },
    },
    ...
    methods: {
      ...
      onClickLabel() {
        const {labelDisabled, disabled, value, proxyValue, proxyDisabled, hasParent} = this;
