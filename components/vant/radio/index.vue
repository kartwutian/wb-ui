...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      labelClasses(){
        return `${this.labelClass} ${utils.bem('radio__label', [this.labelPosition, { disabled: this.isDisabled }])}`
      },
      radioIcon(){
        return `${utils.bem('radio__icon', [this.shape, { disabled: this.isDisabled, checked: this.checked === this.name }])}`
      },
      radioIconStyle(){
        return this.checkedColor && !this.isDisabled && this.checked === this.name ? 'border-color:' + this.checkedColor + '; background-color:' + this.checkedColor + ';' : ''
      },
      radioLabelClass(){
        return `${this.labelClass} ${utils.bem('radio__label', [this.labelPosition, { disabled: this.isDisabled }])}`
      },
