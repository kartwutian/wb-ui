...
function generateApi(allApis, tags){
  ...
  const modelTags = tags.filter(item => item.desc === '前端所属模块').map(item => item.name);
  allApis.forEach((api)=>{
    const currentApi = {
      name: (()=>{ // TODO：生成api的名字，还得改进
...
function generateInitModelData(tags){
  return tags.reduce((prev, next) =>{
    if(next.desc === '前端所属模块'){ // 约定desc字段为“前端所属模块”的name为前端model的名字
      prev[next.name] = []
    }
    return prev;
  }, {__default: []}); // __default 存放默认全局的api,即没有设置tag为前端所属模块的api
}
